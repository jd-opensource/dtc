...
int DTCConfig::parse_config(const char *fn, const char *defsec,
                            bool bakconfig) {
  int ret_code = -1;
  ...
  if (strcmp(sec, "cache") == 0) {
    if (cache_config_[sec]) {
      if (cache_config_[sec][key]) {
        std::string result = cache_config_[sec][key].as<std::string>();
        if (result.length() > 0) {
          val = result.c_str();
        }
      }
    ...
    if (table_config_[sec]) {
      if (table_config_[sec][key]) {
        std::string result = table_config_[sec][key].as<std::string>();
        if (result.length() > 0) {
          val = result.c_str();
        }
      }
...
public:
  ...
  virtual unsigned long long get_size_val(const char *key, const char *inst,
...
AutoConfigSection::~AutoConfigSection() { FREE_CLEAR(section); }
