...
static unsigned short name2port(const char *name, int type)
{
	...
	if (p) {
		...
		localname[len] = 0;
		getservbyname_r(localname, proto, &result_buf, buf, sizeof(buf),
				&result);
	} else {
		getservbyname_r(name, proto, &result_buf, buf, sizeof(buf),
				&result);
	}
	if (result == NULL)
		return 0;
	...
	if (ifr == NULL) {
		...
		if (fd >= 0) {
			...
			if (ioctl(fd, SIOCGIFCONF, &ifc) == 0) {
				ifr = (struct ifreq *)malloc(
					ifc.ifc_len > 128 ? ifc.ifc_len : 128);
				ifc.ifc_req = ifr;
				if (ioctl(fd, SIOCGIFCONF, &ifc) == 0)
					nifr = ifc.ifc_len /
					       sizeof(struct ifreq);
...
static int is_local_ipv4(uint32_t addr)
{
	...
	for (i = 0; i < nifr; i++) {
		if (ifr[i].ifr_addr.sa_family == AF_INET) {
			if (addr == ((struct sockaddr_in *)&ifr[i].ifr_addr)
					    ->sin_addr.s_addr)
				return 1;
		}
	}
...
static int intf2addr(const char *name, uint32_t addr[4])
{
	...
	int i;
	for (i = 0; i < nifr; i++) {
		if (strncmp(ifr[i].ifr_name, name, sizeof(ifr[i].ifr_name)) !=
		    0)
			continue;

		if (ifr[i].ifr_addr.sa_family == AF_INET) {
			addr[0] = ((struct sockaddr_in *)&ifr[i].ifr_addr)
					  ->sin_addr.s_addr;
			return AF_INET;
		}
		...
		if (ifr[i].ifr_addr.sa_family == AF_INET6) {
			memcpy(addr,
			       ((struct sockaddr_in6 *)&ifr[i].ifr_addr)
				       ->sin6_addr.s6_addr,
			       16);
			return AF_INET6;
		}
...
{
	snprintf(sockname, sizeof(sockname), "%c%.*s", un->sun_path[0] ?: '@',
		 (int)sizeof(un->sun_path) - 1, un->sun_path + 1);
}
...
void SocketAddress::build_name_ipv4(void)
{
	...
	snprintf(sockname, sizeof(sockname), "%s:%d/%s", ip,
		 ntohs(in4->sin_port), socktype == SOCK_STREAM ? "tcp" : "udp");
}
...
const char *SocketAddress::set_address(const char *name, const char *portstr)
{
	...
	if (portstr != NULL && portstr[0] != '\0') {
		port = isdigit(portstr[0]) ? atoi(portstr) :
					     name2port(portstr, SOCK_STREAM);
		if (port == 0)
...
const char *SocketAddress::set_address(const struct sockaddr_un *addr, int len,
				       int type)
{
...
const char *SocketAddress::set_address(const struct sockaddr_in *addr, int len,
				       int type)
{
	if (type != SOCK_STREAM && type != SOCK_DGRAM) {
		return "Invalid protocol type\n";
...
const char *SocketAddress::set_address(const struct sockaddr_in6 *addr, int len,
				       int type)
{
	if (type != SOCK_STREAM && type != SOCK_DGRAM) {
		return "Invalid protocol type\n";
...
const char *SocketAddress::set_address(const struct sockaddr *addr, int len,
				       int type)
{
	switch (addr->sa_family) {
	case AF_UNIX:
...
const char *SocketAddress::set_ipv4_address(const char *name, int port,
					    int type)
{
	const char *p = NULL;
...
const char *SocketAddress::set_ipv6_address(const char *name, int port,
					    int type)
{
	const char *p = NULL;
...
const char *SocketAddress::set_host_address(const char *name, int port,
					    int type)
{
	const char *p;
...
int SocketAddress::Equal(const sockaddr *that, int alen, int type) const
{
	...
	case AF_UNIX: {
		...
		if (this->un->sun_path[0] == '/' && un->sun_path[0] == '/') {
			if (!strncmp(this->un->sun_path, un->sun_path,
				     sizeof(un->sun_path))) {
				return 1;
			}
		}
	...
	case AF_INET6:
...

		{
			const struct sockaddr_in6 *in6 =
				(const struct sockaddr_in6 *)that;
			if (this->in6->sin6_port != in6->sin6_port)
				return 0;

			if (!IN6_ARE_ADDR_EQUAL(this->in6->sin6_addr.s6_addr32,
						in6->sin6_addr.s6_addr32))
				return 0;
		}
...
int SocketAddress::Equal(const struct sockaddr_un *addr, int len,
			 int type) const
{
	return Equal((const struct sockaddr *)addr, len, type);
}
int SocketAddress::Equal(const struct sockaddr_in *addr, int len,
			 int type) const
{
	return Equal((const struct sockaddr *)addr, len, type);
}
int SocketAddress::Equal(const struct sockaddr_in6 *addr, int len,
			 int type) const
{
	return Equal((const struct sockaddr *)addr, len, type);
}
...
int SocketAddress::Match(const SocketAddress *that) const
{
	...
	case AF_UNIX:
		...
		if (this->un->sun_path[0] == '/' &&
		    that->un->sun_path[0] == '/') {
			if (!strncmp(this->un->sun_path, that->un->sun_path,
				     sizeof(that->un->sun_path))) {
				return 1;
			}
		}
		break;
	...
	case AF_INET6:
		...
		if (IN6_IS_ADDR_UNSPECIFIED(this->in6->sin6_addr.s6_addr32)) {
			...
			if (that->addr->sa_family == AF_INET6 &&
			    this_port == that->in6->sin6_port) {
				if (IN6_IS_ADDR_UNSPECIFIED(
					    that->in6->sin6_addr.s6_addr32))
					return 1;
...
int SocketAddress::Match(const struct sockaddr_un *addr, int len,
			 int type) const
{
	SocketAddress temp;
...
int SocketAddress::Match(const struct sockaddr_in *addr, int len,
			 int type) const
{
	SocketAddress temp;
	if (temp.set_address(addr, len, type) != NULL)
...
int SocketAddress::Match(const struct sockaddr_in6 *addr, int len,
			 int type) const
{
	SocketAddress temp;
	if (temp.set_address(addr, len, type) != NULL)
