...
int plugin_start(void) {
  ...
  if (main_plugin_mgr->open(
          g_dtc_config->get_int_val("cache", "PluginNetworkMode", 0)) != 0) {
...
int init_cache_mode() {
  g_datasource_mode = g_dtc_config->get_int_val("cache", "DisableDataSource",
                                                DTC_MODE_CACHE_ONLY);
  switch (g_datasource_mode) {
  case DTC_MODE_DATABASE_ADDITION:
    log4cplus_info("dtc datasource mode: %s(%d)", "DTC_MODE_DATABASE_ADDITION",
                   g_datasource_mode);
    break;
  case DTC_MODE_CACHE_ONLY:
    log4cplus_info("dtc datasource mode: %s(%d)", "DTC_MODE_CACHE_ONLY",
                   g_datasource_mode);
    break;
  case DTC_MODE_DATABASE_ONLY:
    log4cplus_info("dtc datasource mode: %s(%d)", "DTC_MODE_DATABASE_ONLY",
                   g_datasource_mode);
    break;
  default:
    log4cplus_error("datasource config error: %d", g_datasource_mode);
    return DTC_CODE_FAILED;
  }
  ...
  if (keyStr == NULL) {
    ...
    log4cplus_error("Can not set "
                    "DisableDataSource=(DTC_MODE_CACHE_ONLY|DTC_MODE_DATABASE_"
                    "ADDITION) and CacheShmKey=NONE together.");
    return DTC_CODE_FAILED;
  ...
  if (g_datasource_mode == DTC_MODE_DATABASE_ONLY && async_update) {
    log4cplus_error("can't DelayUpdate when CacheShmKey set to NONE");
    return DTC_CODE_FAILED;
  }
...
int init_hotbackup_chain_thread() {
  ...
  if (g_hot_backup_thread == NULL || g_hot_backup_ask_instance == NULL) {
    log4cplus_error("hot backup thread or instance created failed.");
    return DTC_CODE_FAILED;
  }
  ...
  if (g_hot_backup_ask_instance->do_init(
          ...
          g_dtc_config->get_size_val("cache", "BinlogOneSize", BINLOG_MAX_SIZE,
                                     'M')) == -1) {
...
int init_buffer_process_ask_chain_thread() {
  ...
  g_buffer_process_ask_instance->set_limit_node_size(
      g_dtc_config->get_int_val("cache", "LimitNodeSize", 100 * 1024 * 1024));
  ...
  if (cache_size <= (50ULL << 20)) // 50M
  {
    ...
    log4cplus_error("CacheMemorySize %lld too large", cache_size);
                 ...
                 cache_size,
                 g_dtc_config->get_int_val("cache", "CacheShmVersion", 4)) ==
             DTC_CODE_FAILED) {
    return DTC_CODE_FAILED;
  }
  ...
  int lruLevel = g_dtc_config->get_int_val("cache", "disable_lru_update", 0);
  if (g_datasource_mode == DTC_MODE_CACHE_ONLY) {
    if (g_buffer_process_ask_instance->enable_no_db_mode() < 0) {
      return DTC_CODE_FAILED;
    }
    if (g_dtc_config->get_int_val("cache", "disable_auto_purge", 0) > 0) {
      g_buffer_process_ask_instance->disable_auto_purge();
      // lruLevel = 3; /* LRU_WRITE */
    }
    int autoPurgeAlertTime =
        g_dtc_config->get_int_val("cache", "AutoPurgeAlertTime", 0);
    g_buffer_process_ask_instance->set_date_expire_alert_time(
        autoPurgeAlertTime);
    if (autoPurgeAlertTime > 0 && TableDefinitionManager::instance()
                                          ->get_cur_table_def()
                                          ->lastcmod_field_id() <= 0) {
      log4cplus_error("Can't start AutoPurgeAlert without lastcmod field");
      return DTC_CODE_FAILED;
    }
  }
  ...
  if (async_update != MODE_SYNC && cache_key == 0) {
    log4cplus_error("Anonymous shared memory don't support DelayUpdate");
    return DTC_CODE_FAILED;
  }

  int iAutoDeleteDirtyShm =
      g_dtc_config->get_int_val("cache", "AutoDeleteDirtyShareMemory", 0);
  ...
  {
    if (TableDefinitionManager::instance()->get_cur_table_def()->uniq_fields() <
        1) {
    ...
    if (g_datasource_mode == DTC_MODE_CACHE_ONLY) {
      if (g_buffer_process_ask_instance->update_mode()) {
        log4cplus_error("Can't start async mode when disableDataSource.");
        return DTC_CODE_FAILED;
      } else {
        log4cplus_error("Can't start disableDataSource with shm dirty,please "
                        "flush async shm to db first or delete shm");
        return DTC_CODE_FAILED;
      }
    } else {
    ...
    /*marker is the only source of flush speed calculattion, inc precision to
     * 10*/
    g_buffer_process_ask_instance->set_flush_parameter(
        g_dtc_config->get_int_val("cache", "MarkerPrecision", 10),
        g_dtc_config->get_int_val("cache", "MaxFlushSpeed", 1),
        g_dtc_config->get_int_val("cache", "MinDirtyTime", 3600),
        g_dtc_config->get_int_val("cache", "MaxDirtyTime", 43200));

    g_buffer_process_ask_instance->set_drop_count(
        g_dtc_config->get_int_val("cache", "MaxDropCount", 1000));
  } else {
    if (g_datasource_mode == DTC_MODE_DATABASE_ADDITION)
      g_data_connector_ask_instance->disable_commit_group();
  }

  if (g_buffer_process_ask_instance->set_insert_order(dbConfig->ordIns) < 0)
    return DTC_CODE_FAILED;
...
int init_remote_dtc_chain_thread() {
  ...
  int connect = g_dtc_config->get_int_val("cache", "HelperConnectTimeout", 10);
...
int init_remote_dtc_chain(PollerBase *thread) {
  ...
  g_remote_dtc_instance = new RemoteDtcAskAnswerChain(
      thread, g_dtc_config->get_int_val("cache", "HelperCountPerGroup", 16));
  ...
  g_remote_dtc_instance->set_timer_handler(thread->get_timer_list(timeout),
...
int init_buffer_process_ask_chain(PollerBase *thread) {
      ...
      g_buffer_process_ask_instance->is_mem_dirty()) // asyncUpdate active
  {
    ...
    case DTC_MODE_DATABASE_ADDITION:
      ...
      break;
    default:
      log4cplus_error("datasource mode error:%d", g_datasource_mode);
      return DTC_CODE_FAILED;
    }
...
int init_data_connector_ask_chain(PollerBase *thread) {
  ...
  g_data_connector_ask_instance->set_timer_handler(
      thread->get_timer_list(timeout), thread->get_timer_list(connect),
      thread->get_timer_list(retry));
...
int init_config_info() {
  ...
  g_target_new_hash = g_dtc_config->get_int_val("cache", "TargetNewHash", 0);
  ...
  log4cplus_info(
      "Table %s: key/field# %d/%d, keysize %d", dbConfig->tblName,
