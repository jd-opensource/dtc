...
bool ConfigHelper::load_dtc_config()
{
	try 
	{
        dtc = YAML::LoadFile(TABLE_CONF_NAME);
	}
...
bool ConfigHelper::load_hot_inst_info()
{
	...
	strncpy(hot_instance.User, table["vhot"]["username"].as<string>().c_str(), sizeof(hot_instance.User)-1 );
	strncpy(hot_instance.Password, table["vhot"]["password"].as<string>().c_str(), sizeof(hot_instance.Password)-1 );
	strncpy(hot_instance.DbName, table["vhot"]["database"].as<string>().c_str(), sizeof(hot_instance.DbName)-1 );
