...
#define DEFAULT_HTTP_TIMEOUT 5 /*default heep connect and request timeout in second*/
...
#define STATS_POOL_CODEC(ACTION)                                                                                    \
    ...
    /* forwarder behavior */                                                                                        \
    ACTION( fragment_error,			       STATS_COUNTER,	   "# times we encountered a fragment error") \
...
enum
{
	...
	AGENT_INOUT_BYTES = 20, //agent request and response bytes. data1 request bytes, data2 response bytes
	CONN_COUNT =100
};
...
void _stats_server_incr(struct context *ctx, struct cache_instance *ins, stats_server_field_t fidx);
void _stats_server_decr(struct context *ctx, struct cache_instance *ins, stats_server_field_t fidx);
void _stats_server_incr_by(struct context *ctx, struct cache_instance *ins, stats_server_field_t fidx, int64_t val);
void _stats_server_decr_by(struct context *ctx, struct cache_instance *ins, stats_server_field_t fidx, int64_t val);
void _stats_server_set_ts(struct context *ctx, struct cache_instance *ins, stats_server_field_t fidx, int64_t val);
